...
def perf_overall(data_dir):
    ...
    # Confusion Matrix
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=classes, yticklabels=classes, ax=axs[0, 0])
    axs[0, 0].set_title('Confusion Matrix')

    # Accuracy Plot
    axs[0, 1].bar(classes, np.mean(all_acc, axis=0))
    axs[0, 1].set_title('Average Accuracy')
    axs[0, 1].set_ylabel('Accuracy')

    # Precision Plot
    for idx, cls in enumerate(classes):
        axs[0, 2].plot(all_precision, label=cls)
    axs[0, 2].set_title('Precision')
    axs[0, 2].set_ylabel('Precision')

    # Recall Plot
    for idx, cls in enumerate(classes):
        axs[1, 0].plot(all_recall, label=cls)
    axs[1, 0].set_title('Recall')
    axs[1, 0].set_ylabel('Recall')

    # Specificity Plot
    for idx, cls in enumerate(classes):
        axs[1, 1].plot(all_specificity, label=cls)
    axs[1, 1].set_title('Specificity')
    axs[1, 1].set_ylabel('Specificity')

    # F1 Score Plot
    for idx, cls in enumerate(classes):
        axs[1, 2].plot(all_f1, label=cls)
